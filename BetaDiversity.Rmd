...
## Beta Diversity
Beta diversity is the extent of change in community composition. Beta diversity is often used to look at compositional heterogeneity. There are many ways to look at beta diversity in statistics. In this code notebook, we will be exploring at the most common ways to look at beta diversity in microbiome analyses. This notebook looks at beta diversity through permanovas, multiple permanovas, NMDS plots, and the betadisper function in R.
...
### Multiple permanova
##### what is multiple permanova versus a normal permanova and what does it show: 
A multiple permanova is a pairwise version of a normal permanova. The key difference between the two being that the multiple permanova runs permanovas for only two of the options at any given time (e.g. Middle Keys vs Upper Keys, Lower Keys vs Upper Keys, Middle Keys vs Lower Keys). So for this data, the multiple permanova would give 3 p-values, one for each variation. A multiple permanovas can show which combination is driving any significance found by a permanova.
##### bonferroni adjustment:
When doing a multiple comparisons test like this one, it is important to include a bonferroni adjustment on your p-values. This adjusts the p-value based on how many tests were performed to help where a single false positive in a series of smaller tests, like the little permanovas run within a multiple permanova, could be an issue.
##### what goes in/out:
